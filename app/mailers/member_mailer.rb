class MemberMailer < ApplicationMailer
  ...
  def standard_member_email(member, subject, body, sending_user, email_id, member_id, perf_set_name, inst, status, set_member_instrument, ensemble_id)
    ...
    mail(to: "#{member.to_s} <#{member.email_1}>",
          ...
          from: "#{reply_to_user_name} via SF Civic Music <no-reply@membermail.sfcivicsymphony.org>",
          subject: subject)
  end
  ...
  def member_signup_email(member, community_night_referral, to_email)
    ...
    mail(to: to_email,
         from: 'SF Civic Music <no-reply@membermail.sfcivicsymphony.org>',
  ...
  def email_finished_email(to_email, email_title, recipient_count)
    mail(to: to_email,
         from: 'SFCMA Membership System <no-reply@membermail.sfcivicsymphony.org>',
